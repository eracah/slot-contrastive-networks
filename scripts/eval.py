...
if __name__ == "__main__":
    ...
    assert args.train_run_dir is not None, "must specify a directory from a run for loading weights!"
    final_dir = Path(args.final_dir)
    train_run_dir = Path(args.train_run_dir)
    train_run_path = final_dir / train_run_dir
    ...
    for probe_type in ["linear", "mlp"]:
        attn_probe = AttentionProbeTrainer(encoder=encoder, epochs=args.epochs, patience=args.patience, lr=args.lr, type=probe_type)
